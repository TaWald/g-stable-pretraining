"""I-JEPA ViT-H/14 pretraining on ImageNet-1k — matches the gijepa reference recipe.

Reference: ``gijepa/configs/in1k_vith14_ep300.yaml``
  model vit_huge (patch 14), epochs 300, warmup 40, ema [0.996, 1.0],
  predictor pred_depth 12 / pred_emb_dim 384, masking num_pred_masks 4
  pred_mask_scale [0.15, 0.2] aspect_ratio [0.75, 1.5] enc_mask_scale [0.85, 1.0],
  lr 2.5e-4 (start_lr 5e-5, final_lr 1e-6), weight_decay 0.04 at effective batch 2048.
  lr is linearly scaled here from the reference batch of 2048.

The gijepa weight-decay ramp 0.04 -> 0.4 is replicated via ``CosineWDSchedule``
(benchmarks/utils.py): the optimizer is created with wd=0.04 and the callback
cosine-ramps it to 0.4 over ``total_steps``.
...
def main():
    ...
    from utils import (
        get_data_dir,
        ijepa_teacher_features,
        maybe_build_seg_eval,
        CosineWDSchedule,
    )
    ...
    # Inline ADE20k kNN-segmentation monitor (frozen backbone). Uses the I-JEPA
    # *target* (teacher/EMA) encoder — the representation evaluated downstream and
    # restored by the post-hoc loader. ViT-H/14 @224 -> 16x16 patch grid.
    seg_eval = maybe_build_seg_eval(
        grid_size=(16, 16),
        feature_fn=ijepa_teacher_features,
