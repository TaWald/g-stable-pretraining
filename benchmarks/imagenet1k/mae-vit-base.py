"""MAE ViT-B/16 pretraining on ImageNet-1k — gmae recipe, ViT-Base backbone.

Same reference recipe as ``mae-vit-large.py`` (gmae: --epochs 800
--warmup_epochs 40 --mask_ratio 0.75 --norm_pix_loss --blr 1.5e-4
--weight_decay 0.05, lr = blr * effective_batch / 256), but on ViT-B/16
(``--model mae_vit_base_patch16`` in the reference).
...
Env knobs: MAX_EPOCHS (default 800), BATCH_SIZE (per-GPU, default 256),
ACCUM (grad accumulation, default 2 -> effective batch 256*8*2=4096 on 8 GPUs,
the MAE reference), CHECKPOINT_DIR, WANDB_DIR, WANDB_ENTITY, WANDB_PROJECT.
"""
...
def main():
    ...
    # effective_batch = per_gpu_batch * num_gpus * accum (reference 4096).
    base_lr = 1.5e-4
    ...
    # ViT-B/16 @224 -> 14x14 patch grid. Env-gated; see maybe_build_seg_eval.
    seg_eval = maybe_build_seg_eval(
        grid_size=(14, 14),
        feature_fn=lambda m, x: m.encoder.forward_features(x)[:, 1:],
        name="mae_vitb_ade20k_seg",
    )
        ...
        callbacks=[
            ...
            *([seg_eval] if seg_eval is not None else []),
            pl.pytorch.callbacks.ModelCheckpoint(
                dirpath=str(Path(ckpt_dir) / "mae-vitb-inet1k"),
